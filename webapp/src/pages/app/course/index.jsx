...
import { Avatar, Button, Collapse, Divider, Empty, Progress } from "antd";
							...
							>
								...
								>
									<div className="p-[8px_20px_8px_20px] bg-white rounded-[40px]">
										<p
											className={`font-bold ${viewType === "list" ? "text-[14px]" : "text-[18px]"}`}
										>
											{item.course?.name}
										</p>
									</div>
								...
								<div
									className={`w-full ${viewType === "list" ? "grid grid-cols-5" : "flex-col"}`}
								>
									<div
									...
									>
										...
										{item.course.settings.show_info_on_course_page && (
											<div
												className={`mt-4 col-span-2 grid ${viewType === "list" ? "grid-cols-4" : "grid-cols-2"} gap-4`}
											>
												{item.course.id_course_certificate && (
													<div className="flex items-center">
														<img
															src={CertificateIcon}
															className="mr-1 max-w-[25px]"
														/>
														<p className="text-[16px]">{t("Certificate")}</p>
													</div>
												)}
												{(item.course.settings?.duration_hours ||
													item.course.settings?.duration_minutes) && (
													<div className="flex items-center">
														<img
															src={DurationIcon}
															className="mr-1 max-w-[25px]"
														...
														<p className="text-[16px]">
															{item.course.settings.duration_hours} h{" "}
															{item.course.settings.duration_minutes
																? `${item.course.settings.duration_minutes}m`
																: ""}
														</p>
													</div>
												)}
												{item.course.settings?.video && (
													<div className="flex items-center">
														...
														/>
														<p className="text-[16px]">
															{item.course.settings.video} videos
														</p>
													</div>
												)}
